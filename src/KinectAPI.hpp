...
public:
	...
	cv::Mat getDepthImage() {return depth;}
...
cv::Mat getDepthMap_src(openni::VideoFrameRef& depth_frame) {
	cv::Mat result = Mat::zeros(depth_frame.getVideoMode().getResolutionY(), depth_frame.getVideoMode().getResolutionX(), CV_8UC3);
	...
	for (int y = 0; y < depth_frame.getVideoMode().getResolutionY(); ++y) {
		for (int x = 0; x < depth_frame.getVideoMode().getResolutionX(); ++x, ++depth_raw) {
			result.at<Vec3b>(y, x)[0] = (int)(*depth_raw / 100);
			result.at<Vec3b>(y, x)[1] = *depth_raw - (int)(*depth_raw / 100) * 100;
...
void KinectAPI::update(int *max_index, cv::Point3d points[]) {
	...
	if (depth_frame.isValid()) {
		depth = getDepthMap_src(depth_frame);
